...
# 找到 Broadleaf Forest 的行
bf_index <- data_sem$FLDTYPCD %in% c("Broadleaf Forest")

# FDis 列
fdis_cols <- c(
  "Shade.tolerance.FDis",
  "Drought.tolerance.FDis",
  "WD_mean_add.FDis",
  "LMA.FDis",
  "Nmass.FDis",
...
existing_fdis_cols <- fdis_cols[fdis_cols %in% colnames(data_sem)]

# 仅在 Broadleaf Forest 内判断异常值
bad_rows <- bf_index &
  (data_sem$AgeGroup %in% c(5)) &
  apply(
    data_sem[, existing_fdis_cols, drop = FALSE],
    1,
    function(x) any(x > 1.6, na.rm = TRUE)
  )

# 删除异常值
data_sem <- data_sem[!bad_rows, ]

# 覆盖原始文件
write.csv(
  data_sem,
  "E:/GEB data/Japan_data_sem_NaturalForests.csv",
  row.names = FALSE
)

cat("删除行数:", sum(bad_rows), "\n")
####620NaturalForests####
TRAITS<-c(
  "Leaf", "Max.height","Shade.tolerance" ,"Drought.tolerance"  ,
  "WD_mean_add",
...
data_sem=read.csv("E:/GEB data/Japan_data_sem_NaturalForests_Broadleaf_filtered.csv" )
data_sem <- data_sem[data_sem$FLDTYPCD %in% c("Broadleaf Forest") , ]
fdis_cols <- c(
...
bad_rows <- (data_sem$AgeGroup %in% c(5)) &
  apply(data_sem[, existing_fdis_cols], 1, function(x) any(x > 1.6, na.rm = TRUE))

# 重要：我们要保留的是“非 bad_rows”的行
data_sem <- data_sem[!bad_rows, ]
