...
load_moran_data <- function(file_path, region, forest_type) {
  if (!file.exists(file_path)) {
    return(NULL)
  }
  read.csv(file_path) %>%
    filter(Trait == "WD_mean_add") %>%
    mutate(Region = region, Type = forest_type)
}

base_path <- "E:/GEB data/"

all_data <- bind_rows(
  # US (American)
  load_moran_data(paste0(base_path, "Moran_Summary_American_220_NaturalForests.csv"), "US", "Conifer"),
  load_moran_data(paste0(base_path, "Moran_Summary_American_630_NaturalForests.csv"), "US", "Mixed"),
  load_moran_data(paste0(base_path, "Moran_Summary_American_620_NaturalForests.csv"), "US", "Broadleaf"),
  # Japan
  load_moran_data(paste0(base_path, "Moran_Summary_Japan_220_NaturalForests.csv"), "Japan", "Conifer"),
  load_moran_data(paste0(base_path, "Moran_Summary_Japan_630_NaturalForests.csv"), "Japan", "Mixed"),
  load_moran_data(paste0(base_path, "Moran_Summary_Japan_620_NaturalForests.csv"), "Japan", "Broadleaf"),
  # Hunan (HN)
  load_moran_data(paste0(base_path, "Moran_Summary_HN_220_NaturalForests.csv"), "Hunan", "Conifer"),
  load_moran_data(paste0(base_path, "Moran_Summary_HN_630_NaturalForests.csv"), "Hunan", "Mixed"),
  load_moran_data(paste0(base_path, "Moran_Summary_HN_620_NaturalForests.csv"), "Hunan", "Broadleaf")
)


age_levels <- c("1-10", "11-30", "31-50", "51-70", "71-120")
age_map <- setNames(age_levels, as.character(1:5))

moran_table <- all_data %>%
  mutate(
    `Age group (years)` = factor(age_map[as.character(AgeGroup)], levels = age_levels),
    Region = factor(Region, levels = c("US", "Japan", "Hunan")),
    Moran_I = round(Moran_I, 3),
    Moran_P = round(Moran_P, 3)
  ) %>%
  dplyr::select(Region, `Age group (years)`, Type, Moran_I, Moran_P) %>%
  
  pivot_wider(
    names_from = Type,
    values_from = c(Moran_I, Moran_P),
    names_glue = "{Type}_{.value}"
  ) %>%
  
  dplyr::select(
    Region, `Age group (years)`,
    Conifer_Moran_I, Conifer_Moran_P,
    Mixed_Moran_I, Mixed_Moran_P,
    Broadleaf_Moran_I, Broadleaf_Moran_P
  ) %>%
  arrange(Region, `Age group (years)`)


print(moran_table)
write.csv(moran_table, "E:/GEB data/Moran_Summary_Table_Combined.csv", row.names = FALSE, na = "NA")

#####Data:Plantations#############################################################
M1=read.csv( "E:/GEB data/Moran_Summary_American_220_Plantations.csv")
M2=read.csv( "E:/GEB data/Moran_Summary_Japan_220_Plantations.csv")
M3=read.csv( "E:/GEB data/Moran_Summary_HN_220_Plantations.csv")
M1$Nation="Amercian"
M2$Nation="Japan"
M3$Nation="HN"
M=rbind(M1,M2,M3)
...
M_filtered <- M %>% filter(Trait == "WD_mean_add")
...
table_result <- M_filtered %>%
  mutate(
    `Age group (years)` = age_map[as.character(AgeGroup)],
    Region = factor(nation_map[Nation], levels = c("US", "Japan", "Hunan")),
    
    Moran_I = round(Moran_I, 3),
    Moran_P = round(Moran_P, 3)
  ) %>%
  dplyr::select(Region, `Age group (years)`, Moran_I, Moran_P) %>%
  arrange(Region, `Age group (years)`)


print(table_result)
write.csv(table_result, "E:/GEB data/Moran_Summary_Table_Combined.csv", row.names = FALSE, na = "NA")
...
data_sem=read.csv("E:/GEB data/FIA_data_sem_NaturalForests.csv" )
data_sem <- data_sem %>%
  mutate(FLDTYPCD = case_when(
    FLDTYPCD == "Conifer Forest"             ~ "CP_General",
    FLDTYPCD == "Conifer-Broadleaf Mixed Forest" ~ "CBMF_General",
    FLDTYPCD == "Broadleaf Forest"           ~ "BMF_General",
    TRUE                                     ~ FLDTYPCD # Keeps other values as is
  ))

result_table <- data_sem %>%
  group_by(AgeGroup,FLDTYPCD) %>%
  dplyr::summarise(
    n = n(),
    STDAGE_mean = round(mean(STDAGE, na.rm = TRUE))
  ) %>%
  arrange(AgeGroup)

result_table
write.csv(result_table,"E:/GEB data/American_PlotNumber_NaturalForests.csv"  )
...
result_table
write.csv(result_table,"E:/GEB data/Japan_PlotNumber_NaturalForests.csv"  )
...
result_table
write.csv(result_table,"E:/GEB data/HN_PlotNumber_NaturalForests.csv"  )

####American####
msd=read.csv("E:/GEB data/Meta_dat1_American_NaturalForests.csv" )

N=read.csv("E:/GEB data/American_PlotNumber_NaturalForests.csv" )

msd=merge(msd,N,by.x= c("AGE", "Types") ,by.y= c("STDAGE_mean", "FLDTYPCD"))
...
N=read.csv("E:/GEB data/Japan_PlotNumber_NaturalForests.csv" )
...
N=read.csv("E:/GEB data/HN_PlotNumber_NaturalForests.csv" )
...
msd$Types <- factor(msd$Types, levels = c("CP_General"
))
msd_5$Types[which(msd_5$Types %in%   c( "220_General" ) )] =  c("CP_General")



msd_5$Types <- factor(msd_5$Types, levels = c("CP_General" ))
...
result_table
write.csv(result_table,"E:/GEB data/American_PlotNumber_Plantations.csv"  )
...
result_table
write.csv(result_table,"E:/GEB data/Japan_PlotNumber_Plantations.csv"  )
...
result_table
write.csv(result_table,"E:/GEB data/HN_PlotNumber_Plantations.csv"  )

####American####

msd=read.csv("E:/GEB data/Meta_dat1_American_Plantations.csv" )
unique(msd$Types)
N=read.csv("E:/GEB data/American_PlotNumber_Plantations.csv" )
...
N=read.csv("E:/GEB data/Japan_PlotNumber_Plantations.csv" )
...
N=read.csv("E:/GEB data/HN_PlotNumber_Plantations.csv" )
